...
rem.Author: Ansh Singh 2021- and Felix Wiegand, 2016-2017
...
for /f %%h in (ServicesW10Ask.inf) do (call:disableService %%h)
endlocal
...
echo.
echo.
echo.System apps configuration
echo.Press "y" and enter to delete...
echo.Press enter to do nothing.
setlocal
set sysAppsRoot=%SystemRoot%\SystemApps
takeown /f %sysAppsRoot% /r /a
icacls %sysAppsRoot% /reset /t
...
rem find /i "%result%" SystemApps.inf >nul
rem if errorlevel 1 goto:rmSystemApp
goto:rmSystemApp
...
rem find /i "%result%" WindowsApps.inf >nul
rem if errorlevel 1 goto:rmWindowsApp
goto:rmWindowsApp
